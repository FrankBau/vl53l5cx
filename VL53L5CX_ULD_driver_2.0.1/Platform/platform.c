...
uint8_t VL53L5CX_WrMulti(
		...
		uint32_t size)
{
	HAL_StatusTypeDef rc = HAL_I2C_Mem_Write(p_platform->hi2c, p_platform->address, RegisterAdress, 2, p_values, size, HAL_MAX_DELAY);
	if(rc != HAL_OK)
		return 255;
...
uint8_t VL53L5CX_RdMulti(
		...
		uint32_t size)
{
	HAL_StatusTypeDef rc = HAL_I2C_Mem_Read(p_platform->hi2c, p_platform->address, RegisterAdress, 2, p_values, size, HAL_MAX_DELAY);
	if(rc != HAL_OK)
		return 255;
